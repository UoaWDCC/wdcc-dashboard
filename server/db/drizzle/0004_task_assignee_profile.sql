...
UPDATE "task_assignee" ta SET "profile_email" = lower(u."email") FROM "user" u WHERE ta."user_id" = u."id";--> statement-breakpoint
DELETE FROM "task_assignee" WHERE "profile_email" IS NULL;--> statement-breakpoint
INSERT INTO "profile" ("email", "name") SELECT DISTINCT lower(ta."profile_email"), ta."profile_email" FROM "task_assignee" ta LEFT JOIN "profile" p ON p."email" = lower(ta."profile_email") WHERE p."email" IS NULL;--> statement-breakpoint

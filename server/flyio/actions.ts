...
export async function listMachinesForApp(
  appName: string,
  slug: string
): Promise<FlyMachine[]> {
  await requireUser();
  ...
  return flyFetch<FlyMachine[]>(
    `${APPS_BASE}/${encodeURIComponent(appName)}/machines`,
    slug
  );
}
...
export async function getMetricsForOrg(
  ...
  for (const { metric, value } of cpu.data.result) {
    ...
    metrics[metric.app] = {
      cpuPercent: Math.round(Number(value[1])),
      memPercent: null,
    };
  }
  for (const { metric, value } of mem.data.result) {
    if (!metric.app) continue;
    metrics[metric.app] = {
      ...(metrics[metric.app] ?? { cpuPercent: null, memPercent: null }),
      memPercent: Math.round(Number(value[1])),
    };
  }

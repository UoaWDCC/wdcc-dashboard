...
export function FlySummary({ orgs }: { orgs: OrgApps[] }) {
  ...
  const avgCpu = average(
    runningApps.flatMap((a) =>
      a.metrics.cpuPercent !== null ? [a.metrics.cpuPercent] : []
    )
  );
  const avgMem = average(
    runningApps.flatMap((a) =>
      a.metrics.memPercent !== null ? [a.metrics.memPercent] : []
    )
  );
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <FlySummaryCard
        ...
        sub={`${uptimePct}% up`}
        subClass={
          running === total ? "text-brand-green" : "text-muted-foreground"
        }
      />

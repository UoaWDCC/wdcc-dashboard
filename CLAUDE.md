...
- Three pause gates in `useBoardSync`: dragging (measured dnd-kit column rects go stale), `useIsMutating()` (the window between the optimistic write and `onSettled`), and an in-flight board fetch. `enabled: !paused` kills the interval; `staleTime: 0` makes un-pause and window focus probe immediately. Drag state therefore lives in `TasksBoard`, not `useTaskDragDrop` — the gate has to be reactive.
